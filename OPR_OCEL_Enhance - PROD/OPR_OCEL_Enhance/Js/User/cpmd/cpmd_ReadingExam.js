...
$(document).ready(function () {
    ...
    var URL = "/question_image/cpmd_doc/" + $('#txt_pdf_file').val();
    ...
    $.confirm({
        ...
        buttons: {
            confirm: {
                ...
                action: function () {
                    $.ajax({
                        url: base_url + "/TaskListExam/FinishReadingBook",
                        type: "POST",
                        data: {
                            RC_ID: $('#txt_record_id').val(),
                            TASK_ID : $('#txt_task_id').val(),
                            COMPETENCY_ID : $('#txt_compentency_id').val()
                        },
                        cache: false,
...
function startTimer(duration, display) {
    ...
    var start = setInterval(function () {
        ...
        if (--timer < 0) {
            clearInterval(start);
            $.ajax({
                url: base_url + "/TaskListExam/updateSelftExamToReady",
                type: 'POST',
                cache: false,
                data: {
                    RC_ID: $('#txt_record_id').val(),
...
function UpdateReminding(RECORD_ID, SISA) {
    $.ajax({
        url: base_url + "/TaskListExam/UpdateRemindingTime",
        type: 'POST',
        cache: false,
        data: {
            R_ID: RECORD_ID,

    ...
    public class SesiUjianController : Controller
    {
        ...
        public JsonResult SesiUjianAction(string TYPE, string TEST_CENTER_ID, string CERTIFICATOR_ID, string DESCRIPTION, string LONG_DESCRIPTION, string MODULE_ID, string MODULE_NAME, string EXAM_START, string EXAM_END, string QUESTION_TYPE, string EVENT_ID, int JENIS_UJIAN_CODE, int EXAM_TARGET_MIN)
        {
            try
            {
                ...
                var cek_ketersediaan_Soal = db_.cufn_cek_ketersediaan_soal(Convert.ToInt32(MODULE_ID), JENIS_UJIAN_CODE.ToString(), Convert.ToInt32(QUESTION_TYPE)).ToList();
                if(cek_ketersediaan_Soal.Count == 0)
                {
                    return this.Json(new { STATUSCODE = 1, MESSAGE_HEADER = "Warning", TYPE = "warning", MESSAGE_BODY = "Untuk Modul, jenis soal dan Tipe ujian tersebut tidak ditemukan" });
                }
